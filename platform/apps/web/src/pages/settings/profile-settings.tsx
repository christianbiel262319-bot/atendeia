...
export function ProfileSettings() {
  ...
  return (
    <div className="grid gap-5">
      <Card className="p-4 sm:p-7"><div className="flex items-start gap-3 sm:gap-4"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-brand-50 text-brand-700 sm:size-12"><UserRound size={23} /></span><div className="min-w-0"><h2 className="font-semibold">Perfil pessoal</h2><p className="mt-2 text-sm leading-6 text-slate-500">O nome identifica suas respostas e ações auditadas.</p></div></div><form className="mt-5 grid gap-4 sm:mt-6 sm:grid-cols-2 sm:gap-5" onSubmit={(event) => void form.handleSubmit((values) => save.mutate(values))(event)}><Field label="Nome completo" error={form.formState.errors.fullName?.message}><Input autoComplete="name" {...form.register("fullName")} /></Field><Field label="E-mail"><Input disabled value={profile?.user.email ?? ""} /></Field><div className="grid gap-3 sm:col-span-2">{save.error ? <ErrorNotice message={save.error.message} /> : null}{save.isSuccess ? <SuccessNotice message="Perfil atualizado." /> : null}<Button className="w-full sm:w-fit" type="submit" disabled={save.isPending || !form.formState.isDirty}>{save.isPending ? "Salvando…" : "Salvar perfil"}</Button></div></form>
      ...
      <Card className="p-4 sm:p-7"><h2 className="font-semibold">Alterar senha</h2><p className="mt-2 text-sm leading-6 text-slate-500">A alteração encerra as outras sessões e mantém somente este dispositivo conectado.</p><form className="mt-5 grid gap-4 sm:mt-6 sm:gap-5" onSubmit={(event) => void passwordForm.handleSubmit((values) => changePassword.mutate(values))(event)}><Field label="Senha atual" error={passwordForm.formState.errors.currentPassword?.message}><Input autoComplete="current-password" type="password" {...passwordForm.register("currentPassword")} /></Field><div className="grid gap-4 sm:grid-cols-2 sm:gap-5"><Field label="Nova senha" error={passwordForm.formState.errors.newPassword?.message}><Input autoComplete="new-password" type="password" {...passwordForm.register("newPassword")} /></Field><Field label="Confirme a nova senha" error={passwordForm.formState.errors.confirmation?.message}><Input autoComplete="new-password" type="password" {...passwordForm.register("confirmation")} /></Field></div>{changePassword.error ? <ErrorNotice message={changePassword.error.message} /> : null}{changePassword.isSuccess ? <SuccessNotice message="Senha alterada e outras sessões encerradas." /> : null}<Button className="w-full sm:w-fit" type="submit" disabled={changePassword.isPending}>{changePassword.isPending ? "Alterando…" : "Alterar senha"}</Button></form></Card>
    </div>
  );
}

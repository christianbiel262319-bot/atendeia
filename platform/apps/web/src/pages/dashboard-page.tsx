...
export function DashboardPage() {
  ...
  const completedFromConditions = data ? Object.values(data.onboarding.steps).filter(Boolean).length : 0;
  const totalSteps = data?.onboarding.total ?? setupSteps.length;
  const completeSteps = Math.min(completedFromConditions, totalSteps);
  const completionPercentage = totalSteps > 0 ? Math.round((completeSteps / totalSteps) * 100) : 0;
  const whatsappConnected = Boolean(data?.onboarding.steps.whatsapp);
  ...
  return (
    <main className="mx-auto w-full min-w-0 max-w-[1500px] px-3 py-4 min-[360px]:px-4 sm:p-7 lg:p-9">
      <div className="mb-5 flex flex-col items-stretch justify-between gap-4 sm:mb-7 sm:flex-row sm:items-end">
        ...
        <span className="inline-flex w-fit items-center gap-2 rounded-full border border-amber-100 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-800"><span className="size-2 rounded-full bg-amber-500" />{whatsappConnected ? "WhatsApp conectado" : "WhatsApp não conectado"}</span>
      </div>
      ...
      <Card className="p-4 sm:p-6">
        <div className="flex items-start justify-between gap-3 sm:gap-5">
          ...
          <div className="shrink-0 text-right text-xs"><strong className="block whitespace-nowrap text-slate-700">{completeSteps} de {totalSteps}</strong><span className="mt-1 block whitespace-nowrap text-[10px] text-slate-400">{completionPercentage}% concluído</span></div>
        </div>
        <div className="my-5 h-1 overflow-hidden rounded-full bg-slate-100"><span className="block h-full rounded-full bg-emerald-700 transition-all" style={{ width: `${completionPercentage}%` }} /></div>

...
function TenantRow({ tenant }: { tenant: Tenant }) {
  ...
  return <div className="p-5"><div className="flex flex-wrap items-start gap-4"><div className="min-w-48 flex-1"><div className="flex flex-wrap items-center gap-2"><strong className="text-sm">{tenant.name}</strong><StatusPill value={tenant.status} /></div><p className="mt-1 text-xs text-slate-400">{tenant.slug} · {tenant._count.memberships} pessoa(s) · {tenant._count.conversations} conversa(s)</p></div><Select className="w-36" value={status} onChange={(event) => setStatus(event.target.value)}><option value="ACTIVE">Ativa</option><option value="SUSPENDED">Suspensa</option><option value="CANCELED">Cancelada</option></Select></div>{status !== tenant.status ? <div className="mt-3 flex flex-col gap-2 sm:flex-row"><Input className="flex-1" placeholder="Motivo obrigatório da alteração" value={reason} onChange={(event) => setReason(event.target.value)} /><Button size="sm" disabled={reason.trim().length < 5 || update.isPending} onClick={() => update.mutate()}>Confirmar</Button></div> : null}{update.error ? <div className="mt-3"><ErrorNotice message={update.error.message} /></div> : null}</div>;
}

function Metric({ icon: Icon, label, value }: { icon: typeof Building2; label: string; value: number | undefined }) { return <Card className="flex items-center gap-3 p-4"><span className="grid size-9 place-items-center rounded-xl bg-emerald-50 text-emerald-700"><Icon size={17} /></span><div><span className="block text-[11px] text-slate-500">{label}</span><strong className="mt-1 block text-xl">{value ?? "—"}</strong></div></Card>; }

  ...
  return (
    <div className="grid gap-5">
      <Card className="p-4 sm:p-7">
        <div className="flex flex-wrap items-start gap-3 sm:gap-4"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-violet-50 text-violet-700 sm:size-12"><ShieldCheck size={23} /></span><div className="min-w-0 flex-1"><h2 className="font-semibold">Autenticação multifator</h2><p className="mt-2 text-sm leading-6 text-slate-500">O MFA é aplicado ao seu acesso a <strong>{profile?.tenant.name}</strong>. Cada código só pode ser utilizado uma vez.</p></div>{profile?.mfaEnabled ? <div className="flex w-full items-center gap-2 sm:w-auto"><span className="inline-flex items-center gap-2 rounded-full bg-brand-50 px-3 py-2 text-xs font-semibold text-brand-700"><CheckCircle2 size={15} /> Ativado</span><Button variant="outline" size="sm" onClick={() => setDisableOpen(true)}>Desativar</Button></div> : <Button className="w-full sm:w-auto" disabled={begin.isPending} onClick={() => begin.mutate()}><KeyRound size={16} />{begin.isPending ? "Preparando…" : "Ativar MFA"}</Button>}</div>

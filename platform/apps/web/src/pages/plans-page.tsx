...
export function PlansPage() {
  ...
  return <Page><PageHeader eyebrow="PLANOS E COBRANÇA" title="Assinatura" description="Escolha um plano cadastrado pela plataforma e conclua a cobrança diretamente no provedor selecionado." />
    ...
    {!plans.isLoading && plans.data?.length === 0 ? <EmptyState title="Nenhum plano disponível" description="O catálogo ainda não foi publicado pelo administrador da plataforma. Nenhum valor fictício será exibido." /> : <div className="grid gap-4 lg:grid-cols-3">{plans.data?.map((plan) => <Card key={plan.id} className="flex min-w-0 flex-col p-4 sm:p-6">{isDemoMode ? <span className="mb-2 w-fit rounded-full bg-amber-50 px-2 py-1 text-[9px] font-bold tracking-wide text-amber-800">VALORES DEMO</span> : null}<span className="break-anywhere text-[10px] font-bold uppercase tracking-[0.14em] text-emerald-700">{plan.code}</span><h2 className="mt-2 text-xl font-semibold">{plan.name}</h2><p className="mt-4 text-2xl font-semibold tracking-tight sm:text-3xl">{money(plan.monthlyPrice, plan.currency)}<span className="text-sm font-normal text-slate-400">/mês</span></p><ul className="my-5 grid flex-1 gap-2 text-sm text-slate-600 sm:my-6">{Object.entries(plan.limits).map(([key, value]) => <li key={key} className="flex min-w-0 items-start gap-2"><span className="mt-1.5 size-1.5 shrink-0 rounded-full bg-emerald-600" /><span className="break-anywhere">{humanize(key)}: <strong className="text-slate-800">{String(value)}</strong></span></li>)}</ul><Button disabled={!canBuy || hasCurrent || capabilities.isLoading || !providerConfigured || checkout.isPending || (provider === "ASAAS" && taxId.replace(/\D/gu, "").length < 11)} onClick={() => checkout.mutate(plan.id)}>{checkout.isPending ? "Abrindo…" : hasCurrent ? "Plano atual definido" : !providerConfigured && !capabilities.isLoading ? "Configuração necessária" : "Escolher plano"}<ExternalLink size={15} /></Button></Card>)}</div>}
  </Page>;
}
